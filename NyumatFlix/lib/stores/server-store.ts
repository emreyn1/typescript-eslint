...
const EMBED_API_URL =
  typeof window !== "undefined"
    ? (window as any).__EMBED_API_URL || process.env.NEXT_PUBLIC_EMBED_API_URL || "http://localhost:3001"
...
export const videoServers: VideoServer[] = [
  {
    ...
    getMovieUrl: (tmdbId) => `${EMBED_API_URL}/watch/movie/${tmdbId}`,
    getTvUrl: (tmdbId) => `${EMBED_API_URL}/watch/tv/${tmdbId}/1/1`,
    getEpisodeUrl: (tmdbId, season, episode) =>
      `${EMBED_API_URL}/watch/tv/${tmdbId}/${season}/${episode}`,
  },
  {
    id: "vidsrc",
    name: "Server 2",
