...
export function normalizeOptions(config, distDir, tempBuildDir) {
    ...
    const { root: monorepoRoot, packager } = findPackagerAndRoot(path.join(process.cwd(), config.appPath || "."));
 ...
 */
export function findPackagerAndRoot(appPath) {
 ...
 */
export function isNextVersionSupported(version) {
 ...
 */
export function checkNextVersionSupport(nextVersion, allowUnsupportedNextVersion, flagName = "--dangerously-use-unsupported-next-version") {
 ...
 */
export function findNextConfig({ appPath, }) {
    const extensions = [
 ...
 */
export function checkRunningInsideNextjsApp({ appPath, }) {
 ...
 * @returns the Next.js runtime used: "webpack" or "turbopack"
 */
export function getBundlerRuntime(options) {
    ...
    if (fs.existsSync(path.join(dotNextServerPath, "chunks/[turbopack]_runtime.js")) ||
