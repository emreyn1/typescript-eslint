...
--------------------------------------------------------------------------------`);
    ...
    readdirSync(standaloneServerDir)
        ...
        .forEach((file) => {
        copyFileAndMakeOwnerWritable(path.join(standaloneServerDir, file), path.join(path.join(outputNextDir, "server"), file));
    ...
    const copyStaticFile = (filePath) => {
        if (existsSync(path.join(standaloneNextDir, filePath))) {
            ...
            copyFileAndMakeOwnerWritable(path.join(standaloneNextDir, filePath), path.join(outputNextDir, filePath));
        }
    };
