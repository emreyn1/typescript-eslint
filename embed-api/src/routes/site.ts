/**
 * Site / landing route — serves a minimal HTML page documenting the public
 * embed API at `GET /`. Mirrors the existing `dist/routes/site.js` pattern but
 * typed in TypeScript and referencing `config.embedDomain`.
 */
...
/**
 * Fastify plugin registering `GET /` — the landing page that documents the
 * public embed API. Matches the existing route style (`embedRoutes`, `streamRoutes`):
 * an exported `async function(app)` registering handlers on the passed instance.
 */
