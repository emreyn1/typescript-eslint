  ...
  useEffect(() => {
    ...
    fetch("/api/referral/stats")
      ...
      .then((data) => {
        ...
        setStats({ directRefs: data.directRefs ?? 0, totalEarned: data.totalEarned ?? 0 });
      })
      .catch(() => {});
  }, [authStatus, router]);
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="bg-gradient-to-br from-emerald-500/20 to-emerald-700/10 border border-emerald-500/30 rounded-xl p-8 mb-8">
          ...
          <p className="text-gray-400 mb-6">
            Share your link. When someone buys a card, you earn commission on every transaction — up to 3 levels deep.
          </p>
          <div className="flex gap-2">
            <input readOnly value={refLink} className="flex-1 bg-black/30 border border-white/10 rounded-lg px-4 py-2.5 text-sm font-mono" />
            <button onClick={copyLink} className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-6">
            ...
            <p className="text-3xl font-bold text-emerald-400">${stats.totalEarned.toFixed(2)}</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left px-6 py-3 text-gray-400 font-medium">Level</th>
                <th className="text-left px-6 py-3 text-gray-400 font-medium">Commission</th>
            ...
            <tbody>
              {COMMISSION_RATES.map((r) => (
                <tr key={r.level} className="border-b border-white/5">
                  ...
                  <td className="px-6 py-3 text-emerald-400 font-medium">{r.rate}</td>
