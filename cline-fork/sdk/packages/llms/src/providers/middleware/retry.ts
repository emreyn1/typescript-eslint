 ...
 *
 * Only classifies errors as retriable when there's strong evidence of a
 * transient condition. Permanent errors (invalid model, auth failures, etc.)
 * are not retried.
 */
export function isRetriableError(error: unknown): boolean {
	...
	if (error && typeof error === "object") {
		...
		if (typeof message === "string") {
			// Use specific patterns to avoid false positives on permanent errors.
			// Broad patterns like /overloaded/i or /capacity/i could match permanent
			// conditions (e.g., "model is at capacity", "internal server error: unknown model").
			// We only match explicit transient phrases that indicate retry is appropriate.
			const retriablePatterns = [
				/rate limit/i,
				/too many requests/i,
				/temporarily unavailable/i,
				/service temporarily overloaded/i,
				/server is temporarily overloaded/i,
				/please retry/i,
				/try again later/i,
			];
 ...
 */
export function createRetryMiddleware(options: RetryOptions = {}): LanguageModelV3Middleware {
				...
				try {
					if (attempt > 0) {
						...
						if (config.onRetryAttempt) {
							await config.onRetryAttempt(attempt, config.maxRetries, delayMs, lastError);
						}
 ...
 *
 * Composes transformParams, wrapStream, and wrapGenerate from all middlewares.
 * For wrapStream and wrapGenerate, middlewares are applied in reverse order
 * (last middleware wraps first, so first middleware is innermost).
 */
		...
		wrapGenerate: async (args) => {
			...
			for (let i = middlewares.length - 1; i >= 0; i--) {
				...
				if (middleware.wrapGenerate) {
					...
					const wrapFn = middleware.wrapGenerate;
