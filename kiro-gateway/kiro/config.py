...
def _load_model_aliases() -> Dict[str, str]:
    """Load MODEL_ALIASES from env JSON, merged with defaults."""
    aliases = dict(_DEFAULT_MODEL_ALIASES)
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            logger.warning("MODEL_ALIASES env var must be a JSON object; using defaults")
    except json.JSONDecodeError as e:
        logger.warning(f"Invalid MODEL_ALIASES JSON: {e}; using defaults")
    return aliases
